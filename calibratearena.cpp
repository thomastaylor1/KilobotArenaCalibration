...
void CalibrateArena::setCalibrationImages(vector<Mat> calImgs)
{
    ...
    {
        cv::cvtColor(*imgsSmall[i], *imgsSmall[i], COLOR_BGR2RGB);
        //IplImage imageIpl = *imgsSmall[i];
        QImage qimg((uchar *) imgsSmall[i]->data,imgsSmall[i]->cols,imgsSmall[i]->rows,QImage::Format_RGB888);
        // TEMPLATE QImage qimg((uchar *) display.data,display.cols, display.rows, display.step,QImage::Format_RGB888);
...
void CalibrateArena::stitcherFinished()
{
    ...
    if (this->thread != NULL) {
        ...
        QImage qimg((uchar *) result.data,result.cols,result.rows, QImage::Format_RGB888);
...
void CalibrateArena::squareArena()
{
    ...
    if (this->thread != NULL && !this->thread->isRunning()) {
        ...
        cv::cvtColor(this->fullSizeFinalIm, this->fullSizeFinalIm, COLOR_BGR2RGB);
        ...
        // create a QImage container pointing to the image data
        QImage qimg((uchar *) shrunkIm.data,shrunkIm.rows, shrunkIm.cols, QImage::Format_RGB888);
...
void CalibrateArena::zoomMove(QPoint pos)
{
    ...
    // create a QImage container pointing to the image data
    QImage qimg((uchar *) s2.data,s2.rows,s2.cols, QImage::Format_RGB888);
